...
def render_login() -> None:
    st.title("Transplantix")
    st.caption("Entrer vos identifiants pour vous connecter")
...
def _metric_card(title: str, value: str, subtitle: str, color: str) -> None:
    st.markdown(
        ...
        ">
            ...
            <div style="font-size:2rem;font-weight:800;color:#0f172a;margin:0.3rem 0;">
                {value}
            </div>
...
def render_sidebar(training_summary: dict[str, Any]) -> None:
    ...
    <div style='text-align:center;'>
        <h2 style='color:white;'>🏥 Transplantix</h2>
        <p style='color:#94a3b8;'>Clinical Decision Support</p>
    </div>
...
def render_result(result: dict[str, Any]) -> None:
    ...
    with left_col:
        ...
        st.dataframe(
            pd.DataFrame(derived_rows, columns=["variable", "value"]),
...
def render_model_analysis(training_summary: dict[str, Any]) -> None:
    ...
    if importance_df.empty:
        st.warning("Aucune importance de variables disponible.")
        return
    ...
    feature_col = find_column(importance_df, ["feature", "Feature", "variable", "Variable"])
    importance_col = find_column(importance_df, ["importance", "Importance", "score"])

    if feature_col is None or importance_col is None:
        st.error(f"Colonnes invalides dans le fichier : {list(importance_df.columns)}")
        return

    top_importance = importance_df[[feature_col, importance_col]].copy()
    top_importance.columns = ["feature", "importance"]

    top_importance = top_importance.head(12)
...
def main() -> None:
    ...
    st.markdown("""
    <h1 style='color:#0f172a;'>🏥 Transplantix</h1>
    <p style='color:#475569; font-size:16px; margin-bottom:20px;'>
        Aide à la décision clinique pour la greffe pédiatrique
    </p>
    """, unsafe_allow_html=True)
    ...
    if page == "form":
        ...
        if input_values["_submitted"]:
            ...
            if validation["errors"]:
                ...
                # 🔥 PASSAGE AUTOMATIQUE À LA PAGE RESULTAT
    ...
    # =========================
        ...
        if result:
            ...
            if st.button("➡️ Analyse du modèle"):
    ...
    # =========================
        ...
        if st.button("➡️ Base de données patient"):
    ...
    # =========================
        ...
        st.subheader("📂 Base de données patient")

        render_history()

        if st.button("🔄 Analyser un nouveau patient"):
            st.session_state["page"] = "form"
            st.rerun()
